...
% SV1 crosses successfully LEFT→MIDDLE→RIGHT from FRONT of OV
% SV2 WAITS in LEFT lane, lets OV pass, then crosses from BEHIND
...
ov_y  = -40.0;         % starts 40m behind SV1
ov_vx = 0.0; 
...
Y_OV_PASSED    = 30.0;  % OV must be this far ahead for rear crossing (INCREASED)
...
for k = 1:N
    ...
    for i = 1:nSV
        ...
        if cur_lane == "LEFT"
            % *** CRITICAL FIX: Different logic for SV1 vs SV2 ***
            
            % Determine if SV is initially AHEAD or BEHIND OV
            initial_ahead_of_ov = (SV(i).y > ov_y);
            
            if initial_ahead_of_ov
                % SV1 case: Crossing from FRONT
                % Can cross if SV maintains lead OR if OV is far behind
                ov_safe = (SV(i).y - ov_y) > Y_SAFE_BEHIND;
                crossing_type = "FRONT";
            else
                % SV2 case: Crossing from BEHIND
                % MUST WAIT until OV has completely passed (ov_clear flag)
                ov_safe = SV(i).ov_clear;
                crossing_type = "BEHIND";
            end
            ...
            if ov_safe
                ...
                if can_change_lane_smart(i, next_lane, SV, ov_x, ov_y, ov_vy, ...
                        ...
                        d_headway_des)
                    start_lane_change(i, next_lane, crossing_type);
                end
            end
            
        elseif cur_lane == "MIDDLE"
            % Goal: MIDDLE → RIGHT
            % More aggressive: attempt even if predecessor is ahead (platoon following)
            next_lane = "RIGHT";
            if can_change_lane_smart(i, next_lane, SV, ov_x, ov_y, ov_vy, ...
