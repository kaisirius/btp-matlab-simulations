function platoon2()
% 3-vehicle platoon vs a MOVING OV with CRPF + VO + DWA (longitudinal only).
...
% RULE: Overtaking allowed from RIGHT lane ONLY. Left lane is forbidden for overtaking.

rng(0); OUT = "outputs_platoon";
...
lane_center_left  = -4.0;   % visually left (−x)
lane_center_right = +4.0;   % visually right (+x)
...
% Longitudinal limits (we disable steering search; ω=0)
a_min = -4.0; a_max = +2.5;
...
ov_y = +20.0;
ov_vx = 0.0;
ov_vy = 10.0;   % moving OV (slower than SVs)
...
% Longitudinal DWA weights (lateral terms removed)
W_RISK  = 1.0;
W_SPEED = 0.08;
W_END   = 0.90;

% Exact recenter parameters (used to "lock" back to MAIN after pass)
HOLD_TICKS = 5;

% Lane-change timing (quintic)
TAU_LC = 2.0;          % duration of lane-change (s)
EPS_CTE = 0.03;        % used for recenter lock counter
EPS_HEAD = deg2rad(2); % not used (theta is straight), but kept for parity

%% --- 3 SVs (changed from 4)
nSV = 3;
...
% Explicit palette: SV1=blue, SV2=red, SV3=orange
palette = [
...
for k=1:N
  ...
  % OV kinematics (moving)
  ov_x = ov_x + ov_vx*dt;
  ov_y = ov_y + ov_vy*dt;

  % --- SIDE DECISION: RIGHT lane ONLY for overtaking. Left lane is forbidden.
  for i=1:nSV
      ...
      if need_consider
          % only consider moving off MAIN if OV is on MAIN
          center_conflict = (SV(i).lane_cur=="MAIN") && (abs(ov_x - lane_center_main) < 1.0);
          if center_conflict
              y0 = SV(i).y - 6; y1 = SV(i).y + 12;
              % Only check RIGHT lane for overtaking (LEFT lane is forbidden)
              freeR = lane_window_free(lane_center_right, y0, y1, ov_x, ov_y, SV, i);

              if freeR
                  % Right lane is free -> move to RIGHT
                  start_lane_change(i, "RIGHT");
              else
                  % Right lane is busy (other SVs ahead) -> stay on MAIN and slow down
                  % (No left lane fallback - just wait on MAIN)
              end
          else
              % Already off MAIN on RIGHT? if far past OV, head back to MAIN
  ...
  % --- LONGITUDINAL DWA for each SV (ω=0; lateral is FSM)
  for i=1:nSV
      vxi = 0;                     % we don't use lateral dynamics
      vyi = SV(i).v;               % motion is along +Y
      % TTC/Risk w.r.t. OV
      ...
      % TTC to predecessor (1D along Y; safe proxy)
      if SV(i).preced>0
         j = SV(i).preced;
      ...
      % Overtake floor if behind and on RIGHT passing lane ONLY
      % (LEFT lane is forbidden so no floor boost if somehow on left)
      is_passing_lane = (SV(i).lane_cur=="RIGHT") || (SV(i).lc_active && SV(i).lane_to=="RIGHT");
      still_behind    = (SV(i).y <= ov_y + Y_BEHIND_PAD);
      if is_passing_lane && still_behind
          v_floor = min(v_max, ov_vy + V_PASS_MARGIN);
          best_v_next = max(best_v_next, v_floor);
      end

      % apply longitudinal
      SV(i).v     = best_v_next;
      SV(i).theta = pi/2;  % keep straight in heading model
  end

  % --- Advance lane-change FSM & integrate Y
  for i=1:nSV
      % update y by longitudinal kinematics
      SV(i).y = SV(i).y + SV(i).v*dt;   % along +Y

      % lane position (x) is dictated by FSM
      if SV(i).lc_active
          ...
          % stay exactly on the center of current lane
  ...
  if k == 1
      fprintf('\n%-6s %-4s %-10s %9s %9s %7s\n', ...
          't[s]','ID','MODE','x[m]','y[m]','lane');
      fprintf('%s\n', repmat('-',1,52));
  end
  for ii = 1:nSV
      fprintf('%-6.1f %-4s %-10s %9.2f %9.2f %7s\n', ...
          t, sprintf('SV%d',ii), char(SV(ii).mode), SV(ii).x, SV(ii).y, char(SV(ii).lane_cur));
  end
  fprintf('%-6.1f %-4s %-10s %9.2f %9.2f %7s\n\n', ...
      t, 'OV', '-', ov_x, ov_y, 'MAIN');
...
%% --- Heatmap anchored at last leave-from-MAIN
...
CR2 = arrayfun(@(x,y) crpf(x,y, anchor_ovx, anchor_ovy, ...
    v_obs_mag, 0,0, m_O, size_O, kappa_O, ...
    CRPF_G, CRPF_ZETA, PSEUDO_EPS, PSEUDO_RHO), GX2, GY2);

figure('Color','w'); hold on;
imagesc(gx2, gy2, log10(CR2+1e-9)); axis xy; colorbar;
title('CRPF Heatmap @ OV pose when last SV starts lane change (moving OV, RIGHT lane only)'); xlabel('X [m]'); ylabel('Y [m]');
...
for i=1:nSV
   plot(log(i).x, log(i).y, SV(i).style, 'Color', SV(i).color, 'LineWidth',2);
...
legend({'SV1','SV2','SV3'},'Location','best');
...
legend({'SV1','SV2','SV3','OV (moving)'}, 'Location','best');
